...
{
    public class FormulaDataService : DataService<DB.Context.Formula, Contracts.Model.Formula,
       Contracts.Model.FormulaFilter, Contracts.Model.FormulaCreator, Contracts.Model.FormulaUpdater>
    {
        ...
        protected override Expression<Func<DB.Context.Formula, bool>> GetFilter(Contracts.Model.FormulaFilter filter)
        {
        ...
        protected override async Task PrepareBeforeAdd(DB.Repository.IRepository<DB.Context.Formula> repository, 
            Contracts.Model.FormulaCreator creator, CancellationToken token)
        {
        ...
        protected override async Task PrepareBeforeUpdate(DB.Repository.IRepository<DB.Context.Formula> repository, 
            Contracts.Model.FormulaUpdater entity, CancellationToken token)
        {
            if (entity.IsDefault)
            {
                var currentDefaults = await repository.GetAsync(new DB.Context.Filter<DB.Context.Formula>()
        ...
        protected override DB.Context.Formula UpdateFillFields(Contracts.Model.FormulaUpdater entity, DB.Context.Formula entry)
        {
