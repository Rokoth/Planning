...
namespace Planning.UnitTests
{
    ...
    public class APIUnitTests
    {
        ...
        {
            var dataService = new Mock<IGetDataService<Contracts.Model.Schedule.Schedule, ScheduleFilter>>();
            var projectSelectService = new Mock<IProjectSelectService>();
            var userSettingsrepo = new Mock<DB.Repository.IRepository<DB.Context.UserSettings>>();
            var logger = new Mock<ILogger<ScheduleApiController>>();

            dataService.Setup(s => s.GetAsync(It.IsAny<ScheduleFilter>(), It.IsAny<CancellationToken>()))
                .Returns(Task.FromResult(new Contracts.Model.Common.PagedResult<Contracts.Model.Schedule.Schedule>(
                    new List<Contracts.Model.Schedule.Schedule>()
                    {

                    }, 10
                    )));

            ScheduleApiController controller = new ScheduleApiController(dataService.Object, projectSelectService.Object, userSettingsrepo.Object, logger.Object);
            ...
            var res = await controller.Get(size: 10, page: 0);
            Assert.True(res is OkObjectResult);
            var result = res as OkObjectResult;
            var actuals = JArray.FromObject(result.Value);
            Assert.Equal(10, actuals.Count);
        ...
        //{
        //    var context = _serviceProvider.GetRequiredService<DB.Context.DbPgContext>();
        //    var direction = CreateDirectionCategory(userId);
        //    context.Set<DB.Context.DirectionCategory>().Add(direction);
        //    await context.SaveChangesAsync();
